...
### KV Cache Reuse
KV cache reuse is supported for MLA on Hopper and Blackwell. It is enabled by default and does not support FP8 KV cache right now. Due to extra operations like memcpy and GEMMs, performance may have regression in some cases. Users could pass `KvCacheConfig(enable_block_reuse=False)` to LLM API to disable it.
